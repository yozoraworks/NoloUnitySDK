    ...
    public class AARPlugin : MonoBehaviour
    {
        ...
        public IEnumerator InitPlugin()
        {
            ...
            for (int i = 0; i < cameraScreens.Length; i++)
            {
                if (cameraScreens[i] != null)
                {
                    bool result = CreateUSBCameraFormat(
                        i, FRAME_FORMAT_DEFAULT, cameraScreens[i].width, cameraScreens[i].height,
                        1, cameraScreens[i].FPS, (float)1 / connectedCameras);
                    CameraDebug.Log("Create camera format " + i.ToString() + " : " + result);
                }
            }
        ...
        public void CreatRawTextureByDeviceID(int i)
        {
            rawTextures[i] = new Texture2D(cameraScreens[i].width,
                cameraScreens[i].height, TextureFormat.RGB24, false, false);
            CameraDebug.Log("create rawTexture");
            rawTextures[i].wrapMode = TextureWrapMode.Clamp;
            rawTextures[i].filterMode = FilterMode.Bilinear;
            androidJavaObject.Call<bool>("setUnityTextureIDByDeviceID", i, (int)rawTextures[i].GetNativeTexturePtr());
        }
        ...
        {
            return androidJavaObject.Call<bool>("CreateUSBCamera", deviceName1, deviceName2, deviceName3, deviceName4,
                true);
        }
        ...
        /// multiple cameras is not recommended for Android device with USB2.0 (due to the limit of transimision bandwidth)
        /// </summary>
        public bool CreateUSBCameraManagerWithRandomDevices(string deviceName1 = "usb/001/002",
            string deviceName2 = "usb/001/004",
        ...
        public Texture2D GetFrame(int deviceID)
        {
            ...
            {
                int textureId = 0;
                if (rawTextures[deviceID] == null ||
                    rawTextures[deviceID].width != cameraScreens[deviceID].width ||
                    rawTextures[deviceID].height != cameraScreens[deviceID].height)
                {
                    rawTextures[deviceID] = null;
                    CreatRawTextureByDeviceID(deviceID);
                }
                androidJavaObject.Call<int>("getTextureID", deviceID, (int)rawTextures[deviceID].GetNativeTexturePtr());
                GL.InvalidateState();
            ...
            else
            {
                    ...
                    blackScreen.height != cameraScreens[deviceID].height)
                {
                    ...
                    blackScreen = new Texture2D(cameraScreens[deviceID].width,
                        cameraScreens[deviceID].height, TextureFormat.RGB24, false, false);
        ...
        public void FromAndroid(string content)
        {
            ...
            if (content.StartsWith("onButtonCallback_"))
            {
                string[] temp = content.Split('_');
                int deviceID = int.Parse(temp[1]);
                int button = int.Parse(temp[2]);
                int state = int.Parse(temp[3]);
            }
